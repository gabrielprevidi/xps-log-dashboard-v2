...
export async function prefiltrar(
  ...
  for (const anexo of email.anexos_xml) {
    ...
    // 3. Fedrigoni, Tecnia e clientes em modo 'especie' contam apenas PDF: o
    //    volume vem do campo QUANTIDADE/ESPÉCIE do DANFE, que não existe no
    //    XML. Descartando aqui, o XML nem chega a ser inserido — e portanto
    //    não "reserva" a chave de acesso, bug que a V1 contorna deletando a
    //    linha depois de gravá-la.
    if ((modo === 'fedrigoni' || modo === 'tecnia' || modo === 'especie') && anexo.nome_arquivo.toLowerCase().endsWith('.xml')) {
      const m = `${modo === 'tecnia' ? 'Tecnia' : modo === 'especie' ? 'Este cliente' : 'Fedrigoni'} usa apenas PDF`
